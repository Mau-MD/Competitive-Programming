...
void solve()
{
    ...
    while(size < n - 1) {
        ...
        // Primero tengo que checar si no hace ciclo
        int grupoA = findGroup(now.a);
        int grupoB = findGroup(now.b);
        if (grupoA != grupoB) {
            group[grupoA] = grupoB;
            costoTotal += now.c;
            size++;
