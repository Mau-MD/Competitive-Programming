...
void solve()
{
    ...
    {
        curr += sweep[i];
        FOR(j,0,sz(intervalos))
        {
            if (i < intervalos[j].fst || i > intervalos[j].snd)continue;
            if (curr == 0)
            {
                ind[j] = i + 1;
                zeros[j]++;
    ...
    bool ok = false;
    FOR(i,0,sz(intervalos))
    {
        if (zeros[i] == 1)
        {
            print(ind[i]); // El numero del segmento donde esta
            ok = true;
        }
    }
