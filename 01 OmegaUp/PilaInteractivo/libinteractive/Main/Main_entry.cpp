...
static struct __stream __PilaInteractivo_in, __PilaInteractivo_out;

void __NOINLINE __libinteractive_init() {
	openstream(&__PilaInteractivo_in, "./PilaInteractivo_in", O_WRONLY);
	openstream(&__PilaInteractivo_out, "./PilaInteractivo_out", O_RDONLY);
}
...
static void __message_loop_PilaInteractivo(int __current_function, int __noreturn) {
	uint32_t __msgid;
	while (readfull(&__PilaInteractivo_out, &__msgid, sizeof(__msgid), 0)) {
...
void agrega(int n) {
	...
	writefull(&__PilaInteractivo_in, &__msgid, sizeof(__msgid));
	writefull(&__PilaInteractivo_in, &n, sizeof(int));
	writefull(&__PilaInteractivo_in, &__cookie, sizeof(__cookie));
	writeflush(&__PilaInteractivo_in);

	__message_loop_PilaInteractivo(__msgid,  0 );
	uint32_t __cookie_result = 0;
	readfull(&__PilaInteractivo_out, &__cookie_result, sizeof(__cookie_result), 1);
...
int saca() {
	...
	__message_loop_PilaInteractivo(__msgid,  0 );
	int __ans = 0;
	readfull(&__PilaInteractivo_out, &__ans, sizeof(__ans), 1);
	uint32_t __cookie_result = 0;
	readfull(&__PilaInteractivo_out, &__cookie_result, sizeof(__cookie_result), 1);
